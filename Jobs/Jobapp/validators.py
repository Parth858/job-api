...
class validationClass:
    ...
    def image_validation(self, image_file):
        ...
        if (
            image_file_extension in allowed_image_extensions and 
            image_file.content_type in allowed_content_types
        ):
            # check signatures
            if ((image_file_extension == "png") and
                (image_file.file.read()[:8].hex().upper().encode("ASCII") == "89504E470D0A1A0A".encode("ASCII"))
            ):
                    upload_file_to_storage = True
            elif (
                (image_file_extension in ["jpg", "jpeg"]) and
