...
class validationClass:
    ...
    def ImageValidation(self, imageFile):
        ...
        if filesize > 10:
            ...
            if imageFileExtension in allowedImageExtensions and imageFile.content_type in allowedContentTypes:
                # check signatures
                if imageFileExtension == "png":
                    if imageFile.file.read()[:8].hex().upper().encode("ASCII") == "89504E470D0A1A0A".encode("ASCII"):
    ...
    def resumeValidation(self, resumeFile):
        ...
        if filesize > 10:
            ...
            if fileExtension in allowedFileExtensions and resumeFile.content_type in allowedContentTypes:
                # check file signature thing
                if fileExtension == "pdf":
                    if resumeFile.file.read()[:4].hex().upper().encode("ASCII") == "25504446".encode("ASCII"):
                        uploadFileToStorage = True
                elif fileExtension == "docx":
                    if resumeFile.file.read()[:4].hex().upper().encode("ASCII") == '504B0304'.encode('ASCII'):
                        uploadFileToStorage = True
                elif fileExtension == "doc":
                    if resumeFile.file.read()[:4].hex().upper().encode("ASCII") == 'D0CF11E0A1B11AE1'.encode('ASCII'):
                        uploadFileToStorage = True
                else:
